...
- **通常モード**（起動時の既定）: 各行に「セットのアイコン（丸いバッジ）」「名前」
  ...
  表示しない）。行タップでチェックリスト画面へ。AppBarに並び替えボタン・編集ボタン・
  表示テーマの選択メニューを表示。右下のFABから新規作成画面へ遷移。新規セットは常に一覧の
  末尾に追加される（`sortOrder` = 作成時点のセット数）。
- **並び替えモード**: AppBarの並び替えボタンをタップすると切り替わる。誤操作を防ぐため、
  各行はアイコン・名前・ドラッグハンドルのみの簡略表示になり、行タップでの画面遷移・FABは
  表示されない。ドラッグハンドルを掴んで並び替えると `TodoSetRepository.reorder` により
  表示順が即座に永続化される。
- **編集モード**: AppBarの編集ボタンをタップすると切り替わる。各行の右端に編集アイコンが
  現れ、そのアイコンをタップしても、行そのものをタップしても
  `TodoSetEditScreen(todoSetId: id)` が開く。FAB・並び替えボタン・テーマメニューは
  表示されない。
- AppBarのアイコンボタン（現在のテーマに応じて表示が変わる、通常モードのみ表示）から
  表示テーマを選択できる（「端末の設定に従う」「ライト」「ダーク」）。選択は
  `themeModeProvider` を通じて即時にアプリ全体へ反映され、Hiveに永続化される。
